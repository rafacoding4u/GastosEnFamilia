    ...
    /*public function crearUsuario()
    {
        try {
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                $idFamilias = recoge('idFamilia'); // Familias seleccionadas
                $idGrupos = recoge('idGrupo'); // Grupos seleccionados
                ...
                if (!empty($nombre_nueva_familia) && !empty($password_nueva_familia)) {
                    if ($m->insertarFamilia($nombre_nueva_familia, $password_nueva_familia)) {
                        ...
                        error_log("Nueva familia creada y asignada como administrador.");
                    } else {
                ...
                foreach ($idFamilias as $idFamilia) {
                    error_log("Intentando asignar a la familia existente con ID: $idFamilia");
                    if ($m->verificarPasswordFamilia($idFamilia, $passwordFamiliaExistente)) {
                        if ($m->asignarUsuarioAFamilia($idUser, $idFamilia)) {
                            error_log("Usuario asignado a la familia con ID: $idFamilia");
                            if ($nivel_usuario === 'admin') {
                                $m->asignarAdministradorAFamilia($idUser, $idFamilia);
                                error_log("Usuario $idUser asignado como administrador a la familia $idFamilia");
                            }
                        } else {
                            error_log("Error al asignar usuario a la familia con ID: $idFamilia");
                            $errores[] = "Error al asignar usuario a la familia con ID: $idFamilia.";
                        }
                    } else {
                        $errores[] = "Contraseña incorrecta para familia ID: $idFamilia.";
                    }
                }
                ...
                if (!empty($nombre_nuevo_grupo) && !empty($password_nuevo_grupo)) {
                    if ($m->insertarGrupo($nombre_nuevo_grupo, $password_nuevo_grupo)) {
                        ...
                        error_log("Nuevo grupo creado y asignado como administrador.");
                    } else {
                ...
                foreach ($idGrupos as $idGrupo) {
                    error_log("Intentando asignar al grupo existente con ID: $idGrupo");
                    if ($m->verificarPasswordGrupo($idGrupo, $passwordGrupoExistente)) {
                        if ($m->asignarUsuarioAGrupo($idUser, $idGrupo)) {
                            error_log("Usuario asignado al grupo con ID: $idGrupo");
                            if ($nivel_usuario === 'admin') {
                                $m->asignarAdministradorAGrupo($idUser, $idGrupo);
                                error_log("Usuario $idUser asignado como administrador al grupo $idGrupo");
                            }
                        } else {
                            error_log("Error al asignar usuario al grupo con ID: $idGrupo");
                            $errores[] = "Error al asignar usuario al grupo con ID: $idGrupo.";
                        }
                    } else {
                        $errores[] = "Contraseña incorrecta para grupo ID: $idGrupo.";
                    }
                }
